...
static const char *address_n_str(const uint32_t *address_n,
  ...
  if (account_type != ACCOUNT_NONE) {
    ...
    if (abbr && accnum < 100) {
      ...
      // account naming:
      // "Legacy", "Legacy SegWit", "SegWit", "Taproot", CoinJoin
      // for BIP44/P2PKH, BIP49/P2SH-P2WPKH, BIP84/P2WPKH, BIP86/P2TR,
      // SLIP25/P2TR respectively. For non-segwit coins we use only BIP44 with
      // no special naming.
      if (legacy) {
